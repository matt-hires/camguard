...
class GDriveStorageAuth:
    """Class for wrapping gdrive authentication
    """
    def __init__(self, settings_file="google-oauth-settings.yaml") -> None:
        """ctor
        ...
        """
        self._gauth: GoogleAuth = GoogleAuth(settings_file=settings_file)
...
class GDriveStorage:
    ...
    def __init__(self, storage_auth: GDriveStorageAuth,
                 upload_folder_title: str = "Camguard"):
        """ctor

        Args:
            storage_auth (GDriveStorageAuth): storage authentication for login
        ...
        self._gdrive: GoogleDrive = GoogleDrive(storage_auth.gauth())
...
class GDriveUploadDaemon(Thread):
    ...
    def __init__(self, storage_auth: GDriveStorageAuth) -> None:
        super().__init__(daemon=True)
        self._stop_event = Event()
        self._queue: Queue[str] = Queue(maxsize=GDriveUploadDaemon._QUEUE_SIZE)
        self._storage_auth: GDriveStorageAuth = storage_auth 
    ...
    def _upload_worker(self) -> None:
        try:
            LOGGER.info("Init")
            storage: GDriveStorage = GDriveStorage(self._storage_auth)
            ...
            LOGGER.error(f"Upload failed: {file} error: {e}")
