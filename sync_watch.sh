...
rsync -avP --rsh=ssh '.' "$remote_host":"$remote_dir" --exclude='venv' \
    --exclude='*.log' --exclude='**/__pycache__' --exclude='.tox/' \
    --exclude='.git/' --exclude='.python-version' --exclude='pip-wheel-metadata/' \
...
--exclude '(\.idea)|(.*~)|(venv)|(\.python-version)|(__pycache__)' . |
